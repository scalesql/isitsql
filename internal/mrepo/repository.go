...
// or attempts to connect and then update the database
// this would only be needed for database upgrades
...
func NewRepository(fqdn, database string, log goose.Logger, rl *appringlog.RingLog) (*Repository, error) {
	...
	if fqdn == "" || database == "" {
		return &r, fmt.Errorf("fqdn and database must be provided")
	}
...
func (r *Repository) handleError(err error) {
	...
	// we just got a different error or a new error
