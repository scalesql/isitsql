...
func TestSetDatabases(t *testing.T) {
	//var zero time.Time
	dbs := []string{"a1", "a2", "a3", "a4"}
	SetDatabases("ag7", dbs)
	_, found := Get("ag7", "a4")
	if !found {
		t.Error("Set didn't set them all")
	}
}

func TestSetDBDelete(t *testing.T) {
	//var zero time.Time
	start := []string{"a1", "a2", "a3", "a4"}
	SetDatabases("ag8", start)
	_, found := Get("ag8", "A4")
	if !found {
		t.Error("Set didn't set them all")
	}
	...
	next := []string{"a5"}
	SetDatabases("ag8", next)
	var f bool

	_, f = Get("ag8", "a1")
	if f {
		t.Error("Shouldn't have a1")
	}
	_, f = Get("ag8", "a2")
	if f {
		t.Error("Shouldn't have a2")
	}
	_, f = Get("ag8", "a3")
	if f {
		t.Error("Shouldn't have a3")
	}
	_, f = Get("ag8", "a4")
	if f {
		t.Error("Shouldn't have a4")
	}
	_, f = Get("AG8", "A5")
	if !f {
		t.Error("Should have a5")
	}
}
